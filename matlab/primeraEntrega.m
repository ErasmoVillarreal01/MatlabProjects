...
yFuncionOriginal = z(1)*puntosX(1).^3 + z(2)*puntosX(1).^2 + z(3)*puntosX(1) + z(4);
yTan =@(x)  0.7485*(x-puntosX(1))+(yFuncionOriginal);
x1 = [puntosX(1):95]; %limite para que mida 80 el largo de las gradas
yGrada1 = yTan(x1); 
yFuncionOriginal = z(1)*puntosX(3).^3 + z(2)*puntosX(3).^2 + z(3)*puntosX(3) + z(4);
yTan =@(x) -0.4300*(x-puntosX(3))+(yFuncionOriginal);
x2 = [puntosX(3):292]; %limite para que mida 80 el largo de las gradas
yGrada2 = yTan(x2);

hold on 
plot(x1-33.37610091,yGrada1,'b','linewidth',10)
plot(x2-50.62935444,yGrada2,'b','linewidth',10)

%para sacar esas distancias las cuales estan restando a x1 y a x2 se saco
%la tangente inversa de la pendiente para tener el angulo con el cual se
%puede despejar la hipotenusa con H = 20/sen(angulo) y así conseguimos la
%distancia

%Aquí se saca la recta tangente con la funcion rectaTangente
pendiente  = rectaTangente(puntosX,fun1,z);
pendiente2  = rectaTangente(puntosX,fun1,z);

velocidadesMax(puntosX, fun1, fun2)
function velMaxs = VelocidadesMax(pos, fun1, fun2) 
primeraZona = [pos(1):pos(2)]
segundaZona = [pos(3):pos(4)]
rCurva = ((1+(fun1(pos)).^2).^(3/2))./(-fun2(pos))
